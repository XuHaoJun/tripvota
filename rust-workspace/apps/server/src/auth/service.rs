...
pub async fn register(
    ...
    if request.email.is_empty() || request.password.is_empty() || request.username.is_empty() {
         ...
         return Err(crate::error::Error::Anyhow(anyhow::anyhow!("Missing fields")));
    }

    // Check if user exists
    let existing_user = accounts::Entity::find()
        .filter(
            accounts::Column::Email.eq(&request.email)
    ...
    new_account.insert(&state.conn).await
        .map_err(|e| crate::error::Error::Anyhow(anyhow::Error::new(e)))?;
...
pub async fn login(
    ...
    let user = accounts::Entity::find()
        .filter(
            accounts::Column::Email.eq(&request.email)
            // .or(accounts::Column::Username.eq(&request.email)) // Uncomment to allow username login
        )
        .one(&state.conn)
        .await
        .map_err(|e| crate::error::Error::Anyhow(anyhow::Error::new(e)))?;
...
pub async fn refresh_token(
    ...
    request: RefreshTokenRequest,
     ...
     let access_token = jwt::sign_token(&claims.sub, secret, 3600)
        .map_err(|e| crate::error::Error::Anyhow(e))?;
