...
# Run cargo lint (fmt + clippy) only for crates that changed since the base ref
...
# If top-level Cargo.toml or workspace Cargo.lock changed, lint all
if git diff --name-only "$BASE"...HEAD -- Cargo.toml Cargo.lock | grep -q .; then
  echo "Workspace manifest/lockfile changed; running lint for all crates"
  cargo fmt --all -- --check
...
if [[ -z "$changed_files" ]]; then
  echo "No Rust changes detected; skipping lint"
  popd >/dev/null
  exit 0
fi
...
if [[ ${#changed_pkgs[@]} -eq 0 ]]; then
  echo "No crate-level changes detected; skipping lint"
  popd >/dev/null
  exit 0
fi

echo "Linting changed crates: ${!changed_pkgs[@]}"
